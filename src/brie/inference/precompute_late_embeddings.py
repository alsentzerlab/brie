"""
One-time precomputation of LateOn (ColBERT) document embeddings for all patients.
...
Usage:
    python precompute_late_embeddings.py -n <notes_dir> -e <embeddings_dir>
"""
...
from .rag_utils import build_late_embeddings_batch, load_cached_late_embeddings
...
def main(args):
    ...
    for fname in sorted(note_files):
        ...
        if load_cached_late_embeddings(patient_id, args.embeddings_dir) is not None:
            continue
        with open(os.path.join(args.notes, fname)) as f:
            uncached[patient_id] = json.load(f)

    if not uncached:
        logger.info("All LateOn embeddings already cached — nothing to do.")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Precompute LateOn ColBERT embeddings for all patients")
    parser.add_argument("-n", "--notes", required=True, help="Directory containing *_subsetrecords.json files")
    parser.add_argument("-e", "--embeddings-dir", required=True, help="Directory to write LateOn embedding cache files")

'''
RAG inference script — dense embedding retrieval (BAAI/bge-small-en-v1.5).

For each question, retrieves the top-K most relevant note chunks using
cosine similarity over pre-computed L2-normalised embeddings, then runs
inference with all three models.

Embeddings are cached per patient in --embeddings-dir as pickle files.
...
from .rag_utils import (
    ...
    encode_queries,
    retrieve_embedding_precomputed,
...
async def run_inference(
    ...
    valid = [index for index, chunk in enumerate(chunks) if (stamp := parse_timestamp(chunk.get("note_date"))) is not None and stamp <= cutoff]
    safe_chunks = [chunks[index] for index in valid]
    safe_embeddings = embeddings[valid]
    retrieved = retrieve_embedding_precomputed(query_emb, safe_chunks, safe_embeddings, top_k=top_k)
...
_EMBEDDING_FIELDS = ["question_id", "model", "approach", "response", "retrieved_chunks"]
...
async def main(args):
    ...
    embedding_cache: dict[str, tuple] = {}  # patient_id -> (chunks, embeddings)
    uncached:        dict[str, list[dict]] = {}  # patients that need encoding
    ...
    for _, row in df.iterrows():
        ...
        if cached is not None:
            ...
            logger.info(f"Loaded cached embeddings for patient {patient_id}")
        else:
            uncached[patient_id] = records

    # Pass 2: encode all uncached patients in one batched model.encode() call
    if uncached:
        logger.info(f"Building embeddings for {len(uncached)} patients in one batch …")
    ...
    for _, row in df.iterrows():
        ...
        if records_cache.get(patient_id) is None:
            ...
            continue
        chunks, embeddings = embedding_cache[patient_id]
        for model in args.models:
            if (str(row["question_id"]), model.removesuffix("_batch")) in completed:
                ...
                continue
            tasks.append((row["question_id"], row[args.question_column], str(row["timestamp"]), chunks, embeddings, model))

    if not tasks:
        logger.info("All tasks already complete.")
        return

    writer = CsvWriter(args.output, _EMBEDDING_FIELDS)

    # Encode all queries in one batched call before the async inference loop
    # (filter tasks first so the query_embeddings index stays aligned)
    logger.info(f"Encoding {len(tasks)} queries …")
    query_embeddings = encode_queries([q for _, q, _, _, _, _ in tasks])  # shape: (N, D)
    ...
    if batch_indices:
        ...
        for i in batch_indices:
            qid, question, query_date, chunks, embeddings, model = tasks[i]
            cutoff = parse_timestamp(query_date)
            if cutoff is None:
                raise ValueError(f"invalid query timestamp: {query_date!r}")
            valid = [index for index, chunk in enumerate(chunks) if (stamp := parse_timestamp(chunk.get("note_date"))) is not None and stamp <= cutoff]
            retrieved = retrieve_embedding_precomputed(query_embeddings[i], [chunks[index] for index in valid], embeddings[valid], top_k=args.top_k)
    ...
    if other_indices:
        ...
        async def delayed(i, question_id, question, query_date, query_emb, chunks, embeddings, model):
            task_delay = 0 if model in VLLM_MODELS else delay
            await asyncio.sleep(i * task_delay)
            result = await run_inference(question_id, question, query_date, query_emb, chunks, embeddings, model, args.top_k, args.backend)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RAG inference with dense embedding retrieval (BAAI/bge-small-en-v1.5)"
    )
    ...
    parser.add_argument("-e", "--embeddings-dir", type=str, required=True,
                        help="Directory for per-patient embedding cache (pickle files)")
